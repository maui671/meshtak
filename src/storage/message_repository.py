...
class MessageRepository:
    ...
    async def get_recent_messages(self, limit: int = 200) -> list[Message]:
        rows = await self._db.fetch_all(
    ...
    async def purge_messages(
        ...
        if channel:
            where.append("(CAST(channel AS TEXT) = ? OR node_name = ?)")
        ...
        if query:
            where.append(
                "(LOWER(text) LIKE ? OR LOWER(node_name) LIKE ? OR LOWER(node_id) LIKE ?)"
            )
        ...
        await self._db.execute(
            f"DELETE FROM messages{where_sql}",
            tuple(params),
        )
        await self._db.commit()
        return deleted
