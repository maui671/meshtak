...
def build_nodeinfo_status(ni) -> dict:
    ...
    if _config is not None and not _config.transmit.enabled:
        ...
        status = "idle"
        reason = (
            "NodeInfo broadcaster is loaded but not running yet. "
            "Restart Meshpoint if this persists."
        )
    else:
        status = "backend_unavailable"
        reason = (
            "Meshtastic TX backend is not ready. Restart Meshpoint after "
...
async def send_nodeinfo_now():
    ...
    if _nodeinfo_broadcaster is None:
        detail = (
            "NodeInfo broadcaster unavailable. "
            "Enable TX and restart Meshpoint, or verify the Meshtastic "
            "radio backend is ready."
        )
        if _config is not None and not _config.transmit.enabled:
            detail = (
                "NodeInfo broadcaster unavailable because TX is disabled. "
                "Enable TX and restart Meshpoint first."
            )
        raise HTTPException(
            503,
            detail,
