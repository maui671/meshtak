    ...
    _wire() {
        ...
        toggle.addEventListener('change', async (e) => {
            ...
            if (result && result.restart_required) {
                const restartMsg = e.target.checked
                    ? 'TX enabled. Restarting Meshpoint now to bring up the radio.'
                    : 'TX disabled. Restarting Meshpoint now to release the radio.';
                this._api.signalRestart(restartMsg);
                this._api.toast('Applying TX change with a service restart');
                await this._api.restartService(restartMsg);
            }
        });
    }
}
